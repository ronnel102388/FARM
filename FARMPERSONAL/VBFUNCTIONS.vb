...
Module VBFUNCTIONS
    ...
    Function ValidateSubfield(ByVal SFID As Integer, ByVal TABLE As String) As Integer
        Dim sql As String = <s>
        SELECT RECID FROM <%= TABLE %> WHERE 
        SUBFIELDID = <%= SFID %> AND ISACTIVE = 1 AND TAG = 1 AND COID = '<%= Comp %>'
                            </s>
